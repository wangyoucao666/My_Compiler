...
void semantic(int reduce_g, const vector<string>& g)
{
	if (reduce_g == 25 || reduce_g == 24)
	{
		// D->id = E ;
		// 先看id是否有类型，若有则判断与E的类型相同与否
		int top = sema.size() - 1;
		if (symbol[sema[top-1]].type == "undefined")
		{
			symbol[sema[top-1]].type = sema.back();
		}
		else
		{
			if (symbol[sema[top-1]].type != sema.back())
			{
				cout << "禁止float与int类型相互赋值或计算" << endl;
				exit(0);
			}
		}
		sema.erase(sema.end() - 1);
	}
	else if (reduce_g == 2)
	{
		// B->A D
		if (symbol[sema[sema.size() - 1]].type == "undefined")
		{
			symbol[sema[sema.size() - 1]].type = sema.back();
		}
		else
		{
			if (symbol[sema[sema.size() - 1]].type != sema[sema.size() - 2])
			{
				cout << "禁止将float与int相互重新定义" << endl;
				exit(0);
			}
		}
		sema.erase(sema.end() - 1);
	...
	else if (reduce_g == 27 || reduce_g == 28 || reduce_g == 30 || reduce_g == 31)
	{
		if (sema.back() != sema[sema.size() - 2])
		{
			cout << "禁止float与int类型相互赋值或计算" << endl;
			exit(0);
		}
	...
	else if (reduce_g == 15 || reduce_g == 17)
	{
		// A->int  A->float
		sema.push_back(g[1]);
	}
	else if (reduce_g == 48 || reduce_g == 49 || reduce_g == 50 || reduce_g == 51)
	{
		sema.erase(sema.end() - 1);
		sema.erase(sema.end() - 1);
	}
...
void analyse()
{
	...
	while (true)
	{
		...
		if (find_nx_move(state, nx_move) == -1)	// 解析下一步操作
		{
			...
			else if (sentence[0].substr(0, 3) == "val")
			{
...
int main()
{
	...
	printf("符号表：\n");
	for (auto&s : symbol)
		cout << s.first<<":"<<s.second.type << endl;
}
